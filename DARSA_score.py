...
class InitialDARSA:
    ...
    def _load_gnn_model(self, model_path: str):
        ...
        model = DARSA(**model_config).to(self.device)
...
class SecondStageDARSA(InitialDARSA):
...
class FinalDARSA(SecondStageDARSA):
    def __init__(self, model_path="best_model.pth", reaction_db="Reaction_USPTO", use_ord=False):
        super().__init__(model_path, reaction_db, use_ord)
        self.gnn_weight = 0
        self.r_weight = 0.8
...
def main(input_csv, output_csv):
    ...
    # Read input from CSV
    try:
        df = pd.read_csv("D:/vcode/DATASET/SA_Data_camp/protac_smiles.csv")
        if 'smiles' not in df.columns or 'chemist_score' not in df.columns:
            raise ValueError("CSV must contain 'smiles' and 'chemist_score' columns")
        
        test_smiles = df['smiles'].tolist()
        chemist_scores = df['chemist_score'].tolist()
        
        print(f"Loaded {len(test_smiles)} molecules from {input_csv}")
    except Exception as e:
        print(f"Error reading input CSV: {e}")
        return
    
    # Initialize predictor with ORD and USPTO
    predictor = FinalDARSA("best_model.pth", reaction_db="Reaction_USPTO", use_ord=True)
    ...
    results_df['rf_only_score'] = rf_only_scores
    ...
    results_df['gnn_only_score'] = gnn_only_scores
    ...
    for smiles in test_smiles:
        ...
        rf_scores.append(result['rf_score'])
        gnn_scores.append(result['gnn_score'])
    
    results_df['combined_score'] = combined_scores
    results_df['rf_component'] = rf_scores
    results_df['gnn_component'] = gnn_scores
    results_df['error'] = results_df['chemist_score'] - results_df['combined_score']
    results_df['abs_error'] = abs(results_df['error'])
    
    # Calculate metrics
    r_squared = calculate_r_squared(chemist_scores, combined_scores)
    mae = calculate_mae(chemist_scores, combined_scores)
    pearson_corr = calculate_pearson(chemist_scores, combined_scores)
    
    # Analyze by complexity
    complex_molecules = results_df[results_df['chemist_score'] > 5]
    simple_molecules = results_df[results_df['chemist_score'] <= 5]
    
    avg_complex_diff = complex_molecules['error'].mean()
    avg_simple_diff = simple_molecules['error'].mean()
    ...
    worst_predictions = results_df.nlargest(5, 'abs_error')
    print("\nTop 5 worst predictions:")
    for i, (idx, row) in enumerate(worst_predictions.iterrows()):
        print(f"{i+1}. Molecule {idx+1}: Chemist={row['chemist_score']:.2f}, Model={row['combined_score']:.2f}, Error={row['error']:.2f}")
...
#     model_scores = []
#     for smi, result in results.items():
#         model_scores.append(result['combined_score'])
